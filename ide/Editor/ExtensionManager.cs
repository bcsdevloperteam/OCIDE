        ...
        public static async Task<List<ExtensionManifest>> GetCatalogAsync()
        {
            try
            {
                using (var client = new System.Net.Http.HttpClient())
                {
                    ...
                    string catalogPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "..", "..", "..", "..", "extenshions", "catalog.json");
                ...
                if (ext != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(ext.Main))
                    {
                        using (var client = new System.Net.Http.HttpClient())
                        {
                            ...
                            // For local development, copy from local Packages directory
                            string localDllPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "..", "..", "..", "..", "extenshions", "Packages", id, ext.Main);
                            if (File.Exists(localDllPath))
                            {
                                File.Copy(localDllPath, Path.Combine(ExtensionsDir, ext.Main), true);
                            }
                        }
                    }
